...
uniform float texture_w3;
uniform float texture_h3;

uniform float ref_amount;

uniform sampler2D Ref_Tex;
uniform sampler2D Ref_Map;
...
void main()
{
    ...
    vec4 ref_col = texture_3point( Ref_Tex, v_vRefTexcoord, vec2(texture_w2,texture_h2) );
    vec4 map_col = texture_3point( Ref_Map, v_vTexcoord, vec2(texture_w3,texture_h3) );

    // map_col.r
    // final value mixing diffuse and reflective
    vec4 final = vec4( mix( dif_col.rgb, ref_col.rgb, (ref_col.a* ref_amount)*map_col.r ),   dif_col.a);
    //(ref_col.a* ref_amount)*map_col.r
